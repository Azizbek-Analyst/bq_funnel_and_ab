...
dropoff_df = bq.analyze_dropoffs(df)
...
```python
# Получение данных воронки с группировкой по A/B-тесту
df = bq.optimized_funnel(
    events=events,
    date_range=date_range,
    window=window,
    group_by='ab_tests.105',  # Группировка по A/B-тесту
    filters={'platform': 'iOS'}
)

# Получение данных для каждой группы
control_df = df[df['group_value'] == 'control']
test_df = df[df['group_value'] == 'test']

# Анализ статистической значимости результатов
significance = bq.analyze_ab_test_significance(
    control_df=control_df,
    test_df=test_df,
    first_step='step1_users',
    last_step='step3_users',
    confidence_level=0.95
)

# Сравнение воронок для контрольной и тестовой групп
bq.compare_funnels(
    dfs=[control_df, test_df],
    labels=['Контрольная группа', 'Тестовая группа'],
    title="Сравнение воронок для A/B-теста"
)
```
